...
func (eh *EnemyHandler) AddEnemy(w http.ResponseWriter, r *http.Request) {
	...
	result, err := eh.EnemyService.AddEnemy(enemy.Nickname, enemy.Life, enemy.Attack, enemy.Defense, enemy.Heal) // Adicionando Heal
	if err != nil {
		switch {
...
func (eh *EnemyHandler) SaveEnemy(w http.ResponseWriter, r *http.Request) {
	...
	result, err := eh.EnemyService.SaveEnemy(id, enemy.Nickname, enemy.Life, enemy.Attack, enemy.Defense, enemy.Heal) // Adicionando Heal
	if err != nil {
		switch {
		case strings.Contains(err.Error(), "internal server error"):
			w.WriteHeader(http.StatusInternalServerError)
